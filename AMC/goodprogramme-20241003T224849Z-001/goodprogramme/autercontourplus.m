for directorynr=1:2%de buitenste for-loop. 
    sizevenster=410;
    disp(directorynr)   
    tic
    ...
    for beeld=1:length(directorycontent)-2%de eerste twee regels van die automatisch gegenereerde directory zijn . en ..
        ...
        if sum(mask(3,:))>0%de scancirkel moet het hele beeld beslaan; in beelden waar dat niet het gaval is, zijn ook geen manuele segmentaties gedaan.
            ...
            topvalue='f';
            for t=2:length(histogram)-1
                if histogram(t-1)<histogram(t) && histogram(t)>histogram(t+1)
                    if topvalue=='f'
                        fpos=t-1;
                        topvalue='l';
                    else lpos=t-1;
            ...
            plot([centers(fpos);centers(fpos)],[0;max(histogram)],'r:')
            plot([centers(lpos);centers(lpos)],[0;max(histogram)],'r:')
            %HET BEPALEN VAN DE THRESHOLD
            %threshold=(centers(fpos)+centers(lpos))/2;
            threshold=centers(find(histogram(fpos+1:lpos+1)==min(histogram(fpos+1:lpos+1)),1,'first')-1+fpos);%de threshold is nu dus het minimum tuseen de pieken
            plot([threshold;threshold],[0;max(histogram)],'b:')
            a=ORIGimage;
