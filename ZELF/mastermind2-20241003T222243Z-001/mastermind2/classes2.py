       ...
       if beurt.counter == 1: return (466)#Dit is code (1,2,3,4)
           ...
           for row in range (2401):
             if overgeblevenCodes.x[row] == 1:
               ...
               if np.average(scores) < minimalValuedCode [0]: 
#                   print("kleiner")
                   minimalValuedCode[0] = np.average(scores)
#                   minimalValuedCode[1] = row
                   placesOfMinimalValue = [row] 
               elif np.average(scores) == minimalValuedCode [0]:
                   #print("even lang")
                   placesOfMinimalValue.append(row)
                   
#    kiest de code die gemiddeld het aantal later mogelijke codes voor alle beoordelingen minimaliseert
           place = random.randint(0, len(placesOfMinimalValue)-1)
#           print("waarvan", len(placesOfMinimalValue), "met gemiddeld", minimalValuedCode [0], "follow-upmogelijkheden" )
           return(placesOfMinimalValue[place])
