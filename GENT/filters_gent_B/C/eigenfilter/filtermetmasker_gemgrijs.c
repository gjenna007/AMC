 ...
 #define maxaantiteraties 7
 #define bottomcontrast 4500
 #define topcontrast 6000
 #define grensgrijswaarde 70
...
/* Load the image specified as the first argument */
   compbeeld =Imlib_load_image(dcompbeeld,"compderuddere03.tif");
   ...
   silhouet=Imlib_load_image(dsilhouet,"silhouetderuddere03.tif");/*Met de functie
	...
	for (ver1=venster[2];ver1<=venster[3];ver1++){
		for (hor1=venster[0];hor1<=venster[1];hor1++){
			...
			if (scanbijhouder[positiekern]==2&&scanbijhouder[positiebuur]==2){
			  int grijsw1=compbeeld->rgb_data[positiekern];
			  int grijsw2=compbeeld->rgb_data[positiebuur];
