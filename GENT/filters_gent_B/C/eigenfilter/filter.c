 ...
 #include <stdio.h>
 #define mask 11
 #define tolerantie 25/*De range waarin de grijswaarden
 ...
 #define mingebiedsgrootte 0
 #define maxgebiedsgrootte 100000
 #define kleurstap 6/*De lengte van een ribbe van de kleurkubus*/
...
/* double BerekenContrast(double*);
   ...
   void InitScanbijhouder(int*,int,int,int);
...
/* Immediately afterwards Intitialise Imlib */
   ...
   dmasker=Imlib_init(disp);
/* Load the image specified as the first argument */
   echtbeeld =Imlib_load_image(dechtbeeld,"derudd05.tif");
   resultaatbeeld=Imlib_load_image(dresultaatbeeld,"derudd05.tif");
   gefilterdbeeld=Imlib_load_image(dgefilterdbeeld,"derudd05.tif");
...
/*Nu maken een randje in "scanbijhouder", om het gebied waarin we de regio's
   ...
   InitScanbijhouder(scanbijhouder,width,height,range);
  ...
  for (hor=range;hor<width-range;hor++){
    if (scanbijhouder[hor+width*ver]==0){
      Filter(hor,ver,width,echtbeeld,resultaatbeeld,gefilterdbeeld,
            ...
            /*contrast,fpinvdiffmoment,fpentropie,*/
...
Imlib_save_image_to_ppm(dresultaatbeeld, resultaatbeeld, "xderudd05.ppm");
Imlib_save_image_to_ppm(dkleurpalet, kleurpalet, "100paletxderudd05.ppm");
Imlib_save_image_to_ppm(dechtbeeld, echtbeeld, "100xderuddfiltered05.ppm");
...
void Filter(int hor, int ver, int width, ImlibImage *echtbeeld,
  ...
  coocmatrix=calloc(256*256,sizeof(double));
   ...
   KleurGebiedje(resultaatbeeld,width,venster,scanbijhouder,kleur);
   ...
   PasCrimminfilterToe(echtbeeld,gefilterdbeeld,width,scanbijhouder,
                       venster,uniformiteit);
...
void KleurGebiedje(ImlibImage *resultaatbeeld, int width, int *venster,
...
for (ver1=venster[2];ver1<=venster[3];ver1++){
  for (hor1=venster[0];hor1<=venster[1];hor1++){
    ...
    if (scanbijhouder[index]==2){
      resultaatbeeld->rgb_data[index*3]=kleur[0]*255/kleurstap;
      resultaatbeeld->rgb_data[index*3+1]=kleur[1]*255/kleurstap;
      resultaatbeeld->rgb_data[index*3+2]=kleur[2]*255/kleurstap;
    }
  }
}
}
...
if (uniformiteit>0.5)
   return 0;
return floor(maxaantiteraties*(5-10*uniformiteit)/4);
}
