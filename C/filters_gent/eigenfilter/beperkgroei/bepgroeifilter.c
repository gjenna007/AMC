 ...
 #define kleurstap 15/*De lengte van een ribbe van de kleurkubus*/
 #define maxaantiteraties 12
 #define bottomcontrast 2000
 #define topcontrast 4500
...
/* We definieren de eerste vulkleur*/
    InitVulkleur(kleur);	
/* En nu gaaan we het filter toepassen */
					...
					for(ver2=-1;ver2<=1;ver2++){
						for (hor2=-1;hor2<=1;hor2++){
							...
							if (scanbijhouder[positie]==0&&
							    abs(blurbeeld[positie]-blurbeeld[hor+width*ver])
								<tolerantie-aantalpunten*tolerantie/maxgebiedsgrootte){
...
return floor((contrast-bottomcontrast)*maxaantiteraties/(topcontrast-bottomcontrast));
}
