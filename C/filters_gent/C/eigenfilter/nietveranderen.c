 ...
 #define mingebiedsgrootte 20
 #define maxgebiedsgrootte 150000
 #define kleurstap 8/*De lengte van een ribbe van de kleurkubus*/
 #define maxaantiteraties 10
 #define bottomcontrast 6000
 #define topcontrast 8000
 #define grensgrijswaarde 300

 /*Hier komen alle functies*/
   void Filter(int,int,int,ImlibImage*,ImlibImage*,ImlibImage*,int*,int*,
                                     int*,FILE*,/*FILE*,FILE*,*/FILE*,
                                     FILE*,FILE*,ImlibImage*,int*,int);
   int  Groeiregio(int,int,int,const int*,int*,int*);
   void Cooc(int,int,int,const ImlibImage*,const int*,const int*,double*);
   double BerekenUniformiteit(const double*);
   double BerekenContrast(const double*);
/* double  BerekenInvdiffmoment(double*);
   double BerekenEntropie(double*);*/
/* double BerekenCorrelatie(int*);*/
   void Beeldblurren(const ImlibImage*,int*,int,int,int);
/*   void Beeldcompressen(ImlibImage*,int*,int,int);*/
   void RGBGelijk(ImlibImage*,int,int);
   void InitScanbijhouder(int*,int,int,int,const ImlibImage*);
   void GebiedDeactiveren(int,const int*,int*);
   void KleurGebiedje(ImlibImage*,int,const int*,const int*,const int*);
   void PasVulkleurAan(int*,double);
   void InitVulkleur(int*);
   int RibbeVakje();
   void TekenKleurpalet(ImlibImage*,const int*,int,const int*);
   void PasKleurpalethorverAan(int*,int);
   int FilterKnop(double,int);
   void PasCrimminfilterToe(ImlibImage*,ImlibImage*,
                           int,const int*,const int*,double,int);
   void Beeldaanpassen(ImlibImage*,const ImlibImage*,int,const int*);
   int BerekenGemGrijswaarde(const ImlibImage*,int,const int*,const int*,int);
...
/* Load the image specified as the first argument */
   compbeeld =Imlib_load_image(dcompbeeld,"detroit.tif");
   resultaatbeeld=Imlib_load_image(dresultaatbeeld,"detroit.tif");
   gefilterdbeeld=Imlib_load_image(dgefilterdbeeld,"detroit.tif");
   kleurpalet=Imlib_load_image(dkleurpalet,"kleurpalet.jpg");
   silhouet=Imlib_load_image(dsilhouet,"witvlak.tif");/*Met de functie
...
/* Eerst zorgen we dat de R, de B, en de G-waarden zeker gelijk zijn*/
   RGBGelijk(compbeeld,width,height);
/*Nu gaan we een gemiddeldewaardefilter op het beeld loslaten,
                               en zodoende "blurbeeld" construeren */
 Beeldblurren(compbeeld,blurbeeld,width,height,range);
 /*  Beeldcompressen(compbeeld,blurbeeld,width,height);*/
...
for (ver=range;ver<height-range;ver++){
  for (hor=range;hor<width-range;hor++){
    if (scanbijhouder[hor+width*ver]==0){
      Filter(hor,ver,width,compbeeld,resultaatbeeld,gefilterdbeeld,
...
Imlib_save_image_to_ppm(dcompbeeld, compbeeld, "c.ppm");
  ...
  coocmatrix=calloc(256*256,sizeof(double));
   ...
   if (aantalpunten>mingebiedsgrootte&&
       aantalpunten<maxgebiedsgrootte){ /*Al te kleine gebieden wordt niets
                                                              mee gedaan*/
     Cooc(hor, ver, width, compbeeld, scanbijhouder, venster, coocmatrix);
     gem_grijswaarde_regio=BerekenGemGrijswaarde(compbeeld,width,scanbijhouder,
                                                         venster,aantalpunten);
     /*KleurGebiedje(compbeeld,width,venster,scanbijhouder,kleur);*/
...
int Groeiregio(int hor,int ver,int width,const int *blurbeeld,int *scanbijhouder,int *venster){
...
void Cooc(int hor,int ver, int width,const ImlibImage *compbeeld,
                              const int *scanbijhouder,const int *venster, double *coocmatrix){
...
void GebiedDeactiveren(int width, const int *venster,int *scanbijhouder){
int hor1,ver1;
for(ver1=venster[2];ver1<=venster[3];ver1++){
  for(hor1=venster[0];hor1<=venster[1];hor1++){
...
double BerekenUniformiteit(const double *coocmatrix){
int hor1,ver1;
double som=0;
for (ver1=0;ver1<256;ver1++){
  for (hor1=0;hor1<256;hor1++){
...
void Beeldblurren(const ImlibImage *compbeeld,int *blurbeeld,int width,
   ...
   while (hor<width-range) {
     ...
     som += compbeeld->rgb_data[index*3+(hor1+ver1*width)*3];
         }
       }
...
void RGBGelijk(ImlibImage*compbeeld,int width,int height){

  int hor,ver,som;
  for (ver=0;ver<height;ver++){
    for (hor=0;hor<width;hor++){
      int index=(hor+width*ver)*3;
      som=compbeeld->rgb_data[index]+compbeeld->rgb_data[index+1]+
                                                compbeeld->rgb_data[index+2];
      compbeeld->rgb_data[index]=compbeeld->rgb_data[index+1]=
                                      compbeeld->rgb_data[index+2]=som/3;
    }
  }
}
...
void KleurGebiedje(ImlibImage *compbeeld, int width, const int *venster,
                                          const int *scanbijhouder, const int *kleur){
int hor1,ver1,index;
for (ver1=venster[2];ver1<=venster[3];ver1++){
  for (hor1=venster[0];hor1<=venster[1];hor1++){
    index=hor1+width*ver1;
    if (scanbijhouder[index]==2){
      compbeeld->rgb_data[index*3]=kleur[0]*255/kleurstap;
      compbeeld->rgb_data[index*3+1]=kleur[1]*255/kleurstap;
      compbeeld->rgb_data[index*3+2]=kleur[2]*255/kleurstap;
    }
  }
}
}
...
void PasVulkleurAan(int *kleur, double contrast){
int temp;
if (contrast>=topcontrast){temp=255;}
if (contrast<=bottomcontrast){temp=0;}
...
void TekenKleurpalet(ImlibImage *kleurpalet,const int *kleurpalethorver,
                                           int kleurpaletafmeting,const int *kleur){
...
void PasCrimminfilterToe(ImlibImage *resultaatbeeld,ImlibImage *gefilterdbeeld,
                          int width,const int *scanbijhouder,const int *venster,
                          double contrast,  int gem_grijswaarde_regio){
...
void Beeldaanpassen(ImlibImage *resultaatbeeld, const ImlibImage *gefilterdbeeld,
                                                     int width,const int *venster){
...
int BerekenGemGrijswaarde(const ImlibImage *compbeeld, int width, const int *scanbijhouder,
                                            const int *venster, int aantalpunten){
...
/*
void Beeldcompressen(ImlibImage *compbeeld,int *blurbeeld,int width,
                                              int height){
...
for (ver=0;ver<height;ver++){
  for(hor=0;hor<width;hor++){
    index=hor+ver*width;
    blurbeeld[index]=compbeeld->rgb_data[index*3];
  }
}
}
