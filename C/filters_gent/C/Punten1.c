...
for (USUint i=0; i<nr_columns;i++){
  ...
  sprintf(temp,"%d",(USUint) i);// Maakt een character van de variabele i
...
for(USUintversie=0;versie<nr_columns;versie++){
  for(USUint column=0;column<nr_columns;column++){
    //Read the stalactiet from disk
    sprintf(temp,"%d",(USUint) column);
    tissuename=stalactiet+temp;
