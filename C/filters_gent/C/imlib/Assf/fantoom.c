...
for (hor1=0;hor1<w;hor1++){
  ...
  for (ver1=0;ver1<h;ver1++){
    gemiddelde=(im->rgb_data[(hor1+w*ver1)*3]
		+im->rgb_data[(hor1+w*ver1)*3+1]
...
/* Geheugenruimte reserveren voor de verschillende tabellen */
    mean = malloc(w*h*sizeof(double));/* mean en alpha zijn iets 
					 te groot, maar vanwege de
					 manier waarop ik ze invul moet dat ook */
    alpha = malloc(w*h*sizeof(double));
    betadomein = malloc(lookupsize*sizeof(double));/* deze kunnen
						      wel precies groot genoeg 
						      gemaakt worden */
    betabeeld = malloc(lookupsize*sizeof(double));
    mergeomgeving = malloc((3*mask)*(3*mask)*sizeof(char));
    mergebijhouder = malloc((2*mask+1)*(2*mask+1)*sizeof(char));
    pixelomgeving = calloc(w*h*(mask+2)*(mask+2),sizeof(char));

/* De pixelomgevingen initialiseren */

for (hor1=0;hor1<w*h;hor1++)
  pixelomgeving[(hor1*(mask+2)*(mask+2)+centralpixel)*sizeof(char)]=1;
  ...
  int ver2, hor2, waarde=0, noemer=0;
    ...
    for (ver2=1;ver2<=mask;ver2++){
      for (hor2=1; hor2<=mask;hor2++){
        if (pixelomgeving[((hor1+w*ver1)*(mask+2)*(mask+2)+hor2+(mask+2)*ver2)*sizeof(char)]==1){
...
/* Hier gaan we alvast anticiperen op het tweede deel van het filter. 
   ...
   gesproken is die op 1 geïnitialiseerd). */
    pixelomgeving[((hor1+w*ver1)*(mask+2)*(mask+2)+centralpixel)*sizeof(char)]=noemer;
...
Imlib_save_image_to_ppm( id2, im2,"halffantoombeeld.ppm");
...
printf("%d",hor1);
  for(ver1=mask;ver1<=h-mask;ver1++){
    if (pixelomgeving[((hor1+w*ver1)*(mask+2)*(mask+2)+centralpixel)*sizeof(char)]>kvanmerge){
                              /*Hier wordt gecheckt of het gebiedje groot genoeg is om in het
                                                        mergeproces te worden opgenomen*/
       int ver2,hor2,waarde=0,noemer=0;
       for(ver2=0;ver2<3*mask;ver2++){
         for(hor2=0;hor2<3*mask;hor2++){
           mergeomgeving[(ver2*3*mask+hor2)*sizeof(char)]=0;
         }
       }
       mergeomgeving[((mask+range)*3*mask+mask+range)*sizeof(char)]=1;
       for(ver2=0;ver2<2*mask+1;ver2++){
         for(hor2=0;hor2<2*mask+1;hor2++){
           mergebijhouder[(ver2*3*mask+hor2)*sizeof(char)]=0;
         }
       }
       mergebijhouder[(mask*(2*mask+1)+mask)*sizeof(char)]=1;
/* In het array mergeomgeving wordt gewoon het gebied 
   bijgehouden waarover de smoothoperator gaat werken. (Het gebied wat je krijgt als je de
verschillende gebieden merget dus.*/
/* In mergebijhouder wordt bijgehouden wat de zaadpixels zijn van de 
   gebieden waarmee gemerged wordt. Dit om het bepalen van het 
   gebied waarover gesmoothed
   moet worden te versnellen. Als we bij een pixel besluiten 
   dat een bepaald gebied in het mergeproces betrokken moet worden,
   dan zetten we de corresponderende zaadpixel in de mergebijhouder op  1. 
   Als we nu via een andere pixel tegen datzelfde gebied aanlopen, dan kunnen we zien dat dat
gebied al 'aan de beurt' is geweest*/
     for(ver2=1;ver2<=mask;ver2++){
       for(hor2=1;hor2<=mask;hor2++){
         if (pixelomgeving[((hor1+w*ver1)*(mask+2)*(mask+2)+hor2+ver2*(mask+2))*sizeof(char)]==1){
           mergeomgeving[((mask+ver2)*3*mask+mask+hor2)*sizeof(char)]=1;
	   ...
	   mask+teller,mask+1,teller,1,mergeomgeving,mergebijhouder,
	   w,range,centralpixel);
           Scan(hor1-1,ver1,pixelomgeving,mask+hor2,mask+ver2,mask-1,
	   mask,-1,0,mergeomgeving,mergebijhouder,w,range,centralpixel);
           Scan(hor1+1,ver1,pixelomgeving,mask+hor2,mask+ver2,mask+1,mask,
	   1,0,mergeomgeving,mergebijhouder,w,range,centralpixel);*/
           }
          }
         }
       }
   ...
   for (ver2=0;ver2<3*mask;ver2++){
     for (hor2=0; hor2<3*mask;hor2++){
       if (mergeomgeving[(ver2*3*mask+hor2)*sizeof(char)]==1){
  ...
  im->rgb_data[(hor1+w*ver1)*3+2]=waarde/noemer;
   ...
   Imlib_save_image_to_ppm( id, im,"resultaatfantoombeeld.ppm");
   free(alpha);
   free(mean);
   free(betadomein);
   free(betabeeld);
...
for(hor3=-range;hor3<=range;hor3++){
  for(ver3=-range;ver3<=range;ver3++){/* Dit is precies het gebied waar pixels kunnen zitten wiens omgeving de pixel die nu bekeken wordt kan bevatten*/
    if(mergebijhouder[(((xmergeomg-range-1)+hteller+hor3)*(2*mask+1)+(ymergeomg-range-1)+vteller+ver3)*sizeof(char)]==0){
      if(pixelomgeving[((xomgevingspixel+hor3+w*(yomgevingspixel+ver3))*(mask+2)*(mask+2)+centralpixel-hor3+(mask+2)*(centralpixel-ver3))*sizeof(char)]!=0){
      int ver4,hor4;
        for(ver4=1;ver4<=mask;ver4++){
          for(hor4=1;hor4<=mask;hor4++){
            if(pixelomgeving[((xomgevingspixel+hor3+w*(yomgevingspixel+ver3))*(mask+2)*(mask+2)+hor4+(mask+2)*ver4)*sizeof(char)]!=0)
              mergeomgeving[((xmergeomg+hteller+hor3-range-1+(hor4-1))*3*mask+ymergeomg+vteller+ver3-range-1+(ver4-1))*sizeof(char)]=1;
          }
        }
     mergebijhouder[(((xmergeomg-range-1)+hteller+hor3)*(2*mask+1)+(ymergeomg-range-1)+vteller+ver3)*sizeof(char)]=1;
    }
  }
}
}
